...
-- REQUIRED
	harpoon:setup()
-- REQUIRED
   --vim.api.nvim_set_keymap('n', '<leader>a', ':lua print("Test")<CR>', { noremap = true, silent = true })

--vim.g.mapleader = "\\"
--remove this mapleader lol 
	vim.keymap.set("n", "<leader>a", function() harpoon:list():add() end)
	vim.keymap.set("n", "<M-e>", function() harpoon.ui:toggle_quick_menu(harpoon:list()) end)

    --	vim.keymap.set("n", "<C-h>", function() harpoon:list():select(1) end)
   	vim.keymap.set("n", "<M-j>", function() harpoon:list():select(1) end) -- Need to rebind from ctrl-h. Ctrl-h is window navigation.
